...
class GaussianModel:
    ...
    def __init__(self, feat_dim: int=32, 
        ...
        # ============================================================
        # Hierarchical Context Injection (HCI) - residual MLP
        # Conditions each anchor's feature on a parent-self-child 
        # context triplet to establish cross-level gradient pathways.
        # Input: [self_feat, parent_level_mean, child_level_mean] -> residual
        # ============================================================
        self.mlp_hci = nn.Sequential(
    ...
    # def precompute_level_means(self):
    #     """
    #     Compute per-level mean features over the FULL anchor set and cache
    #     a single pre-projection needed by compute_hci. Idempotent within
    #     a training step.
    #     """
    #     feat = self._anchor_feat                           # [N_total, d]
    #     levels_flat = self._level.squeeze(1).long()        # [N_total]
 
    #     num_levels = int(levels_flat.max().item()) + 1
    #     d = self.feat_dim
    #     device, dtype = feat.device, feat.dtype
 
    #     # Per-level sum and count via scatter_add (one pass over all anchors)
    #     level_sums = torch.zeros(num_levels, d, device=device, dtype=dtype)
    #     level_cnts = torch.zeros(num_levels, 1, device=device, dtype=dtype)
    #     idx_expand = levels_flat.unsqueeze(1).expand_as(feat)
    #     level_sums.scatter_add_(0, idx_expand, feat)
    #     level_cnts.scatter_add_(
    #         0, levels_flat.unsqueeze(1),
    #         torch.ones(feat.shape[0], 1, device=device, dtype=dtype),
    #     )
    #     level_means = level_sums / level_cnts.clamp(min=1)   # [L, d]
 
    #     # Pre-project level_means through the parent- and child-halves
    #     # of the first Linear layer. After this, per-anchor context
    #     # retrieval is just a gather — no [N, d] intermediate needed.
    #     linear1 = self.mlp_hci[0]        # nn.Linear(3d, hidden)
    #     W1 = linear1.weight              # [hidden, 3d]
    #     # Columns [0:d]   -> applied to self_feat
    #     # Columns [d:2d]  -> applied to parent_ctx
    #     # Columns [2d:3d] -> applied to child_ctx
    #     W_par = W1[:, d:2 * d]           # [hidden, d]
    #     W_chi = W1[:, 2 * d:]            # [hidden, d]
 
    #     # [L, d] @ [d, hidden] = [L, hidden]. L is small (≤10), so this
    #     # matmul is essentially free.
    #     self._hci_par_proj_cache = level_means @ W_par.t()   # [L, hidden]
    #     self._hci_chi_proj_cache = level_means @ W_chi.t()   # [L, hidden]
    #     self._hci_max_level = num_levels - 1
 
    # ------------------------------------------------------------------
    # Fast path called per camera. Mathematically equivalent to original
    # HCI when level_means is computed over the full anchor set.
    # ------------------------------------------------------------------
    # claude认为有问题版本，先注释，下面是修复版本
    # def compute_hci(self, feat, levels, visible_mask=None):
    #     """
    #     Hierarchical Context Injection with SACA.
    
    #     Args:
    #         feat:         [N_vis, d]  visible anchor features (with grad)
    #         levels:       [N_vis, 1]  integer level indices
    #         visible_mask: [N_total] bool  (optional) enables SACA filtering
    
    #     Returns:
    #         [N_vis, d] feat + per-level constant delta
    #     """
    #     if feat.shape[0] == 0:
    #         return feat
    
    #     d = self.feat_dim
    #     levels_flat = levels.squeeze(1).long()
    
    #     unique = torch.unique(levels_flat)
    #     unique_cpu = unique.cpu().tolist()
    #     num_unique = len(unique_cpu)
    
    #     # Edge case: single level visible
    #     if num_unique == 1:
    #         self_ctx = feat.mean(dim=0, keepdim=True)
    #         context_input = torch.cat([self_ctx, self_ctx, self_ctx], dim=1)
    #         delta = self.mlp_hci(context_input)
    #         return feat + delta.expand_as(feat)
    
    #     # -------- SACA setup --------
    #     use_saca = visible_mask is not None
    #     if use_saca:
    #         if not getattr(self, '_saca_cache_valid', False):
    #             self.rebuild_saca_masks()
    #         # Subset the global masks to visible anchors
    #         vis_act_parent = self._saca_is_active_parent[visible_mask]  # [N_vis]
    #         vis_act_child = self._saca_is_active_child[visible_mask]    # [N_vis]
    
    #     # -------- Compute per-level means (plain + filtered) --------
    #     self_means_list = []   # plain mean (used as self context)
    #     par_source_list = []   # filtered mean (used as parent context for the next level)
    #     chi_source_list = []   # filtered mean (used as child context for the previous level)
    
    #     for lv_val in unique_cpu:
    #         mask = (levels_flat == lv_val)
    #         lvl_feat = feat[mask]
    #         plain_mean = lvl_feat.mean(dim=0)
    #         self_means_list.append(plain_mean)
    
    #         if use_saca:
    #             par_mask = vis_act_parent[mask]
    #             if par_mask.any():
    #                 par_source_list.append(lvl_feat[par_mask].mean(dim=0))
    #             else:
    #                 par_source_list.append(plain_mean)  # fallback
    
    #             chi_mask = vis_act_child[mask]
    #             if chi_mask.any():
    #                 chi_source_list.append(lvl_feat[chi_mask].mean(dim=0))
    #             else:
    #                 chi_source_list.append(plain_mean)
    #         else:
    #             par_source_list.append(plain_mean)
    #             chi_source_list.append(plain_mean)
    
    #     self_means = torch.stack(self_means_list)       # [L, d]
    #     par_source = torch.stack(par_source_list)       # [L, d]
    #     chi_source = torch.stack(chi_source_list)       # [L, d]
    
    #     # -------- Build parent/child slot indices --------
    #     lv_to_slot = {lv: i for i, lv in enumerate(unique_cpu)}
    #     unique_set = set(unique_cpu)
    
    #     parent_slots = torch.empty(num_unique, dtype=torch.long, device=feat.device)
    #     child_slots = torch.empty(num_unique, dtype=torch.long, device=feat.device)
    #     for i, lv_val in enumerate(unique_cpu):
    #         par_lv = lv_val - 1 if (lv_val - 1) in unique_set else lv_val
    #         chi_lv = lv_val + 1 if (lv_val + 1) in unique_set else lv_val
    #         parent_slots[i] = lv_to_slot[par_lv]
    #         child_slots[i] = lv_to_slot[chi_lv]
    
    #     # Parent context for level L comes from level L-1's par_source mean
    #     # Child context for level L comes from level L+1's chi_source mean
    #     parent_ctx = par_source[parent_slots]           # [L, d]
    #     self_ctx = self_means                           # [L, d]
    #     child_ctx = chi_source[child_slots]             # [L, d]
    #     context_input = torch.cat([parent_ctx, self_ctx, child_ctx], dim=1)
    
    #     level_deltas = self.mlp_hci(context_input)      # [L, d]
    
    #     anchor_slot = torch.empty_like(levels_flat)
    #     for i, lv_val in enumerate(unique_cpu):
    #         anchor_slot[levels_flat == lv_val] = i
    #     delta_per_anchor = level_deltas[anchor_slot]
    
    #     return feat + delta_per_anchor
    def compute_hci(self, feat, levels, visible_mask=None):
        """
        HCI with Weighted SACA: soft structure-aware context aggregation.
    
        Key change: instead of filtering anchors by (is_active_parent / child),
        use a soft weight: 1.0 for active, alpha for inactive. Compute weighted
        mean. This avoids the noisy small-sample problem when active ratio is low.
        """
        if feat.shape[0] == 0:
            return feat
        ...
        # -------- Weighted SACA setup --------
        use_saca = visible_mask is not None
        alpha = getattr(self, '_saca_alpha', 0.3)  # soft weight for inactive anchors 默认03
    
        if use_saca:
            if not getattr(self, '_saca_cache_valid', False):
                self.rebuild_saca_masks()
            vis_act_parent = self._saca_is_active_parent[visible_mask]
            vis_act_child = self._saca_is_active_child[visible_mask]
    
        # -------- Compute per-level means (plain + weighted) --------
        self_means_list = []
        par_source_list = []
        chi_source_list = []
        ...
        for lv_val in unique_cpu:
            ...
            if use_saca:
                # As-parent: soft weight based on is_active_parent
                par_lvl_mask = vis_act_parent[mask]
                par_weight = torch.where(par_lvl_mask,
                                        torch.ones_like(par_lvl_mask, dtype=feat.dtype),
                                        torch.full_like(par_lvl_mask, alpha, dtype=feat.dtype))
                par_weighted_sum = (lvl_feat * par_weight.unsqueeze(1)).sum(dim=0)
                par_weight_sum = par_weight.sum().clamp(min=1e-6)
                par_source_list.append(par_weighted_sum / par_weight_sum)
    
                # As-child: soft weight based on is_active_child
                chi_lvl_mask = vis_act_child[mask]
                chi_weight = torch.where(chi_lvl_mask,
                                        torch.ones_like(chi_lvl_mask, dtype=feat.dtype),
                                        torch.full_like(chi_lvl_mask, alpha, dtype=feat.dtype))
                chi_weighted_sum = (lvl_feat * chi_weight.unsqueeze(1)).sum(dim=0)
                chi_weight_sum = chi_weight.sum().clamp(min=1e-6)
                chi_source_list.append(chi_weighted_sum / chi_weight_sum)
            else:
                par_source_list.append(plain_mean)
                chi_source_list.append(plain_mean)
    
        self_means = torch.stack(self_means_list)
        par_source = torch.stack(par_source_list)
        chi_source = torch.stack(chi_source_list)
        ...
        parent_ctx = par_source[parent_slots]
        self_ctx = self_means
        child_ctx = chi_source[child_slots]
        context_input = torch.cat([parent_ctx, self_ctx, child_ctx], dim=1)
    
        level_deltas = self.mlp_hci(context_input)
    ...
    # ============================================================
    # PATCH 2: Add rebuild_saca_masks method to GaussianModel
    # ============================================================
    
    def rebuild_saca_masks(self):
        """
        Build boolean masks marking which anchors participate in cross-level
        relationships:
        _saca_is_active_parent[i] = True if anchor i has any child at level i_level+1
        _saca_is_active_child[i]  = True if anchor i has a valid parent at level i_level-1
    
        Called after each densification (tree structure is static between
        densifications).
        """
        with torch.no_grad():
            N = self._anchor.shape[0]
            device = self._anchor.device
            levels_flat = self._level.squeeze(1).long()
            max_level = int(levels_flat.max().item())
            ...
            for L in range(1, max_level + 1):
                cur_mask = (levels_flat == L)
                par_mask = (levels_flat == L - 1)
                ...
                cur_pos = self._anchor[cur_mask]
                par_pos = self._anchor[par_mask]
    
                # Parent voxel size at level L-1
                par_voxel_size = self.voxel_size / (float(self.fork) ** (L - 1))
    
                cur_par_grid = torch.round((cur_pos - self.init_pos) / par_voxel_size).long()
                par_grid = torch.round((par_pos - self.init_pos) / par_voxel_size).long()
            ...
            self._saca_is_active_parent = is_active_parent
            self._saca_is_active_child = is_active_child
            self._saca_cache_valid = True
    
            # Diagnostic (can remove after verifying)
            # if True:
            #     total_par = (levels_flat < max_level).sum().item()
            #     total_chi = (levels_flat > 0).sum().item()
            #     act_par_ratio = is_active_parent.sum().item() / max(total_par, 1)
            #     act_chi_ratio = is_active_child.sum().item() / max(total_chi, 1)
            #     print(f"[SACA] active_parent: {act_par_ratio:.2%}, active_child: {act_chi_ratio:.2%}")
 
    # ------------------------------------------------------------------
    # Invalidate the cache whenever anchor_feat may have been modified
    # outside of optimizer.step() (e.g., after densification).
    # ------------------------------------------------------------------
    # def invalidate_hci_cache(self):
    #     self._hci_par_proj_cache = None
    #     self._hci_chi_proj_cache = None
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            # HCI MLP: use the same lr schedule as mlp_cov
            {'params': self.mlp_hci.parameters(), 'lr': training_args.mlp_cov_lr_init, "name": "mlp_hci"},
        ]
        if self.appearance_dim > 0:
        ...
        # HCI MLP scheduler: reuse mlp_cov lr schedule
        self.mlp_hci_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_cov_lr_init,
                                                    lr_final=training_args.mlp_cov_lr_final,
                                                    lr_delay_mult=training_args.mlp_cov_lr_delay_mult,
                                                    max_steps=training_args.mlp_cov_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group['lr'] = lr
            if param_group["name"] == "mlp_hci":
                lr = self.mlp_hci_scheduler_args(iteration)
                param_group['lr'] = lr
    ...
    def save_mlp_checkpoints(self, path):
        ...
        if  args.distributed_save:
            ...
            # HCI MLP
            hci_mlp = torch.jit.trace(self.mlp_hci, (torch.rand(1, self.feat_dim * 3).cuda()))
            hci_mlp.save(os.path.join(path,  "_rk"
                    + str(utils.GLOBAL_RANK)+'hci_mlp.pt'))
            if self.use_feat_bank:
            ...
            param_dict['hci_mlp'] = self.mlp_hci.state_dict()
            if self.use_feat_bank:
    ...
    def load_mlp_checkpoints(self, path, mode = 'split'):
        ...
        if args.distributed_save:
            ...
            # HCI MLP
            hci_path = os.path.join(path, "_rk" + str(utils.GLOBAL_RANK)+'hci_mlp.pt')
            if os.path.exists(hci_path):
                self.mlp_hci = torch.jit.load(hci_path).cuda()
            if self.use_feat_bank:
            ...
            # HCI MLP
            if 'hci_mlp' in checkpoint:
                self.mlp_hci.load_state_dict(checkpoint['hci_mlp'])
            if self.use_feat_bank:
    ...
    def _load_additional_attributes(self, folder):
        attributes_path = os.path.join(folder, 'additional_attributes.npz')
        if not os.path.exists(attributes_path):
            raise FileNotFoundError(
                f"Missing {attributes_path}. "
                "This checkpoint does not contain init_pos required by HCI/SACA render."
            )

        load_npz = np.load(attributes_path)
        if 'init_pos' not in load_npz:
            raise KeyError(
                f"{attributes_path} does not contain 'init_pos'. "
                "Please re-save the checkpoint with init_pos or disable the SACA path."
            )

        self.init_pos = torch.tensor(load_npz['init_pos'], dtype=torch.float, device="cuda")
    ...
    def distributed_load_ply(self, folder):
        ...
        self._load_additional_attributes(folder)
    ...
    def all2all_tensors_in_optimizer_implementation_1(self, destination, i2j_send_size):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if group["name"] == "mlp_opacity" or group["name"] == "mlp_cov" or group["name"] == "mlp_color" or group["name"] == "mlp_hci":
                continue
